...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('Le Van Nhat XP BF Premium') then
        makefolder('Le Van Nhat XP BF Premium')
    end
    if not _G.Dis and not isfile('Le Van Nhat XP BF Premium/'..Name..'.json') then
        writefile('Le Van Nhat XP BF Premium/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('Le Van Nhat XP BF Premium') and isfile('Le Van Nhat XP BF Premium/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('Le Van Nhat XP BF Premium/'..Name..'.json'))
...
function Text(value)
    game.StarterGui:SetCore("SendNotification", {
        Title = "Thông Báo!", 
        Text = tostring(value),
        Icon = "http://www.roblox.com/asset/?id=12867501021",
        Duration = 10
    })
end
function Com()
...
if IKAI then
	...
	function library:Window(text,text2,text3,logo,keybind)
		...
		ToggleImgUi.Image = "rbxassetid://12867501021"
	...
	spawn(function()
		while wait() do
			pcall(function()
				...
				local tempo = string.format("%.0f Giờ , %.0f Phút , %.0f Giây", hours ,minutes, seconds)
	...
	ImageLabelfps.Image = "rbxassetid://12867501021"
	...
	ImageLabeltime.Image = "rbxassetid://12867501021"
...
game.StarterGui:SetCore("SendNotification", {
	Title = "Thông Báo!", 
	Text = "Chào Mừng Bạn Đã Đến Script\nLe Van Nhat XP Hub.",
	Icon = "http://www.roblox.com/asset/?id=12874752177",
	Duration = 8
})

local win = library:Window("Switch",[[Hub]],[[Phiên Bản: 2.7 ]],"12867501021",Enum.KeyCode.RightControl)
local General_Tab = win:Tab("Trang Chủ",[[7040391851]])
local Quest_Tab = win:Tab("    Nhiệm vụ & Item",[[9606626859]])
local PvP_Tab = win:Tab("PvP",[[9606626034]])
local Raid_Tab = win:Tab("Raid",[[9606629300]])
local Shop_Tab = win:Tab("Cửa Hàng",[[9606625251]])
local Island_Tab = win:Tab("Đảo",[[9606628205]])
local Setting_Tab = win:Tab("Cài Đặt",[[9606644121]])
local Status_Tab = win:Tab("Thống Kê",[[9613645002]])
local Esp_Tab = win:Tab("Định Vị",[[9606628205]])
local Hop_Tab = win:Tab("Hop",[[9608089732]])
General_Tab:Label("Treo/Cày Level | Le Van Nhat XP")

General_Tab:Toggle("Tự Động Cày Level","9606294253",_G.Setting_table.AutoFarm,function(vu)
	Auto_Farm = vu
...
if _G.Setting_table.FastAttack_Mode == nil then
	_G.Setting_table.FastAttack_Mode = "Nhanh"
end
MIo = {
	"Siêu Nhanh",
	"Nhanh",
...
General_Tab:Toggle("Đánh Nhanh ","9606294253",_G.Setting_table.FastAttack,function(vu)
...
General_Tab:Dropdown("Đánh Nhanh","Nhanh",MIo,function(vu)
	_G.Setting_table.FastAttack_Mode = vu
	Update_Setting(getgenv()['MyName'])
	if _G.Setting_table.FastAttack_Mode == "Nhanh" then
		_G.Fast_Delay = 0.1
	elseif _G.Setting_table.FastAttack_Mode == "Bình Thường" then
		_G.Fast_Delay = 0.3
	elseif _G.Setting_table.FastAttack_Mode == "Siêu Nhanh" then
		_G.Fast_Delay = 0
	end
end)
...
local Select_W = General_Tab:Dropdown("Chọn Vũ Khí",_G.Setting_table.Weapon,Waspon,function(vu)
	_G.Setting_table.Weapon = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Button("Làm Mới Vũ Khí",function(vu)
	Select_W:Clear()
	for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do  
		if v:IsA("Tool") then
			Select_W:Add(v.Name)
...
General_Tab:Label("Tự Động Giết Boss")
General_Tab:Toggle("Tự Động Giết","9610159123",_G.Setting_table.Farm_Boss,function(vu)
	Auto_Farm_Boss = vu
...
General_Tab:Toggle("Tự Động Giết Tất Cả Boss","9610159123",_G.Setting_table.Farm_All_Boss,function(vu)
...
General_Tab:Label("Tự Động Giết Quái")
General_Tab:Toggle("Tự Động Giết Quái","9606294253",_G.Setting_table.Auto_Farm_Monster,function(vu)
...
local Select_M = General_Tab:Dropdown("Chọn Quái",_G.Setting_table.Mon_Name,Mons,function(vu)
	_G.Setting_table.Mon_Name = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Button("Làm Mới Quái",function(vu)
	Select_M:Clear()
	local xx = {}
...
General_Tab:Label("Tự Động Cày Thông Khạo")
General_Tab:Toggle("Tự Động Cày Thông Khạo Trái Ác Quỷ","9606294253",_G.Setting_table.Farm_Fruit,function(vu)
	Auto_Farm_Fruit = vu
...
General_Tab:Slider("Máu %",1,30,_G.Setting_table.MinHealth,function(vu)
	MinHealth = vu
...
General_Tab:Toggle("Kỹ Năng X","9606294253",_G.Setting_table.Skill_X,function(vu)
	SkillX = vu
	_G.Setting_table.Skill_Z = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Kỹ Năng C","9606294253",_G.Setting_table.Skill_C,function(vu)
	SkillC = vu
	_G.Setting_table.Skill_Z = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Kỹ Năng V","9606294253",_G.Setting_table.Skill_V,function(vu)
	SkillV = vu
	_G.Setting_table.Skill_Z = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Tự Động Cày Melee","9606294253",_G.Setting_table.Farm_Melee,function(vu)
	Auto_Farm_Melee = vu
...
General_Tab:Toggle("Tự Động Cày Vũ Khí","9606294253",_G.Setting_table.Farm_Sword,function(vu)
	Auto_Farm_Sword = vu
...
General_Tab:Label('Cày Xương')
General_Tab:Toggle("Tự Động Cày Xương","9606294253",_G.Setting_table.Farm_Bone,function(vu)
	Auto_Farm_Bone = vu
...
General_Tab:Toggle("Tự Động Random Xương,"9606294253",_G.Setting_table.Farm_Random_S,function(vu)
	Farm_Random_S = vu
...
General_Tab:Label('Haki Quan Sát')
General_Tab:Toggle("Tự Động Cày Haki Quan Sát","9606294253",_G.Setting_table.Farm_Ken,function(vu)
	Farm_Ken = vu
...
General_Tab:Label("Tự Động Lấy Vật Phẩm")
General_Tab:Toggle("Tự Động Mở Haki Màu","9606294253",_G.Setting_table.Farm_Ob_Color,function(vu)
	Open_Color_Haki = vu
...
General_Tab:Toggle("Tự Động Lấy Đuốc","9606294253",_G.Setting_table.Auto_Holy_Torch,function(vu)
	Auto_Holy_Torch = vu
	_G.Setting_table.Auto_Holy_Torch = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Tự Động Bay Tới chest","9606294253",_G.Setting_table.Grab_chest,function(vu)
	Grab_chest = vu
	_G.Setting_table.Grab_chest = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Tự Đông Nhặt chest Đừng Khi Có Đuốc","9606294253",_G.Setting_table.STP,function(vu)
	Grab_chest = vu
	T_P_H = vu
	_G.Setting_table.STP = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Khoá Số chest","9606294253",_G.Setting_table.Hop_chest,function(vu)
	_G.Setting_table.Hop_chest = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Slider("Khoá chest",1,100,_G.Setting_table.K_MAX,function(vu)
	_G.Setting_table.K_MAX = vu
	Update_Setting(getgenv()['MyName'])
end)
local K_C = General_Tab:Label('chest Đã Khoá : 0')
                ...
                if T_P_H then
                    if game.Players.LocalPlayer.Backpack:FindFirstChild("Fist of Darkness") or game.Players.LocalPlayer.Character:FindFirstChild("Fist of Darkness") then
						...
						if _G.chest_100 ~= nil then
							...
							if Grab_chest then
								K_CH = K_CH + 1
								K_C:Set('Rương : '..tostring(K_CH).."/".._G.Setting_table.K_MAX)
							end
						...
						elseif _G.chest_10500 ~= nil then
							...
							if Grab_chest then
								K_CH = K_CH + 1
								K_C:Set('Ruơng : '..tostring(K_CH).."/".._G.Setting_table.K_MAX)
							end
...
General_Tab:Label("Tự Động Thống Kê")
General_Tab:Toggle("Melee","9606294253",_G.Setting_table.Melee_A,function(vu)
	Melee_A = vu
	_G.Setting_table.Melee_A = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Phòng Thủ","9606294253",_G.Setting_table.Defense_A,function(vu)
	Defense_A = vu
	_G.Setting_table.Defense_A = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Kiếm","9606294253",_G.Setting_table.Sword_A,function(vu)
	Sword_A = vu
	_G.Setting_table.Sword_A = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Trái Ác Quỷ","9606294253",_G.Setting_table.Fruit_A,function(vu)
	Fruit_A = vu
	_G.Setting_table.Fruit_A = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Toggle("Súng","9606294253",_G.Setting_table.Gun_A,function(vu)
	Gun_A = vu
	_G.Setting_table.Gun_A = vu
	Update_Setting(getgenv()['MyName'])
end)
General_Tab:Slider("Điểm",1,100,3,function(vu)
	Points = vu
end)
General_Tab:Button("Sao Chép",function()
				...
				elseif game.Players.LocalPlayer.Backpack:FindFirstChild("Fist of Darkness") or game.Players.LocalPlayer.Character:FindFirstChild("Fist of Darkness") then
					...
					_G.chest_100 = nil
						...
						if _G.chest_100 ~= nil then
							...
							if Auto_Dark_Coat then
								C_H_H = C_H_H + 1
								L_C:Set('Rương : '..tostring(C_H_H).."/".._G.Setting_table.chest_Lock)
							end
						elseif _G.chest_500 ~= nil then
							repeat wait(.3)
								TP(_G.chest_500.CFrame)
